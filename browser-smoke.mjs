...
const healthResponse=await fetch('http://127.0.0.1:4310/api/health',{headers:{authorization:`Bearer ${key}`}}),health=await healthResponse.json();assert.equal(healthResponse.status,200);assert.equal(health.supervised,true);assert.equal(health.status,'healthy');
const browser=await chromium.launch({executablePath:'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',headless:true});
try {
  const context=await browser.newContext({viewport:{width:1440,height:1000}});
  const page=await context.newPage(); const errors=[];page.on('pageerror',e=>errors.push(e.message));
  await page.goto('http://127.0.0.1:4310');
  await page.locator('#token').fill(key);await page.locator('#connect-form button[type="submit"]').click();
  ...
  await page.locator('#create').waitFor({state:'hidden'});await page.waitForFunction(()=>document.getElementById('model-settings')?.disabled===false);
  ...
  await phone.goto('http://127.0.0.1:4310');await phone.waitForFunction(()=>document.getElementById('model-settings')?.disabled===false);
  ...
  await page.reload();await page.waitForFunction(()=>document.getElementById('model-settings')?.disabled===false);
