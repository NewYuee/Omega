...
export type FeishuQuote={messageId:string;authorId:string;authorType:string;text:string;parentId?:string;resources?:FeishuResource[];attachments?:FeishuAttachment[]};
...
export function parseFeishuQuote(message:Record<string,any>|undefined,messageId:string,chatId:string):FeishuQuote{
  ...
  if(!['text','image','file'].includes(message.msg_type))throw Error('引用仅支持文本、图片和文件消息；富文本、语音、视频、卡片及合并转发暂不支持');
  ...
  return{messageId,authorId:typeof sender?.id==='string'?sender.id.slice(0,100):'未知',authorType:typeof sender?.sender_type==='string'?sender.sender_type.slice(0,30):'未知',text,...(parentId?{parentId}:{}),...(resources.length?{resources}:{})};
}
...
export function withFeishuQuoteChain(question:string,chain:FeishuQuoteChain){
  ...
  const parts=chain.messages.map((quote,index)=>{
    const {resources,attachments,...info}=quote;
