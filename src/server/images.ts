...
export const IMAGE_TTL=7*24*60*60*1000,MAX_IMAGE_BYTES=8*1024*1024,MAX_IMAGES=4;
...
export class ImageStore{
  ...
  async turnInput(input:ImagePart[],ids:string[]=[]){
    ...
    for(const value of input)for(const part of splitImageText(value.text!,ids)){if(part.type==='text')output.push(part);else{if(++count>MAX_IMAGES)throw imageError('每条消息最多 4 个图片块');used.add(part.id);output.push({type:'localImage',path:await this.resolve(part.id)});}}
    for(const id of ids)if(!used.has(id)){if(++count>MAX_IMAGES)throw imageError('每条消息最多 4 个图片块');output.push({type:'localImage',path:await this.resolve(id)});}
    return output;
  }
}
