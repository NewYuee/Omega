...
export const MAX_PASTED_TEXTS=MAX_MESSAGE_TEXT_FILES;
...
export class PastedTextStore{
  ...
  async refs(ids:string[]=[]){if(!Array.isArray(ids)||ids.length>MAX_PASTED_TEXTS||new Set(ids).size!==ids.length)throw pastedTextError('每条消息最多包含 20 段不同的粘贴文本');return Promise.all(ids.map(async id=>{const {text,...ref}=await this.resolve(id);return ref;}));}
