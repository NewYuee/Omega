...
import {isMissingThreadHistory} from '../shared/thread-errors.ts';
...
export class GroupOrchestrator{
  ...
  async recoverTask(task:Row){
    ...
    }catch(error){if(this.store.getTask(task.id)?.status==='cancelled')return;const detail=errorMessage(error),missing=isMissingThreadHistory(error)||/找不到对应执行轮次/i.test(detail);this.store.failTask(task.id,missing?'成员会话或执行轮次已无法恢复，请编辑成员关联或手动重试':detail,missing?'failed':'unknown');this.changed(task.groupId);}
  ...
  async dispatchTask(groupId:string,raw:Row){
    ...
    try{
      ...
      submitted=true;
      const started=await this.startTurn(member.threadId,prompt,dispatchId,{cwd:member.cwd,accessMode:mode,projectScope:member.projectName,imageIds:(req.images||[]).map((image:Row)=>image.id)}),turnId=started.turn?.id;
      ...
      const missing=isMissingThreadHistory(error);
