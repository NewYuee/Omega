...
try{
  for(const width of [390,1280]){
    ...
    await page.addInitScript(()=>{sessionStorage.setItem('omega-key','mock-group-key');const original=window.fetch;window.fetch=(url,options)=>String(url).includes('/api/events')?Promise.resolve(new Response(new ReadableStream({start(c){window.roomEvent=e=>c.enqueue(new TextEncoder().encode('data: '+JSON.stringify(e)+'\n\n'));}}))):original(url,options);});
    ...
    await page.locator('.omega-palette-trigger').waitFor({state:'attached'});await page.waitForFunction(()=>typeof window.omegaAppState?.getSnapshot()?.shellActions?.switchMode==='function');assert.equal(await page.locator('body').evaluate(node=>node.classList.contains('omega-product')),true);await page.keyboard.press('Control+K');assert.equal(await page.locator('.omega-palette').isVisible(),true);await page.keyboard.press('Escape');
    await page.waitForTimeout(50);assert.deepEqual(errors,[]);
    if(width<701){await page.locator('#menu-toggle').click();const drawerState=await page.evaluate(()=>({open:document.getElementById('conversation-drawer').open,handler:typeof window.omegaAppState?.getSnapshot()?.shellActions?.switchMode,sidebarParent:document.getElementById('sidebar').parentElement?.id}));assert.equal(drawerState.open,true,JSON.stringify(drawerState));assert.equal(drawerState.handler,'function');assert.equal(await page.locator('#show-groups').isVisible(),true);await page.locator('#show-groups').click();await page.locator('#groups .group-row').click();}
    ...
    assert.equal(await page.locator('#group-view').isVisible(),true);assert.equal(await page.locator('#composer').isVisible(),false);const more=page.locator('#group-more'),togglePanel=async selector=>{if(await more.getAttribute('open')===null)await more.locator('summary').click();await page.locator(selector).click()};await more.locator('summary').click();assert.equal(await page.locator('#group-concurrency').inputValue(),'3');assert.equal(await page.locator('.group-delete').isVisible(),true);
    ...
    assert.ok((await page.locator('#requirement-form').boundingBox()).height<=125);
    const centerWidth=(await page.locator('.group-center').boundingBox()).width;if(width<=800){await togglePanel('.member-panel-toggle');assert.equal(await page.locator('.group-panel').first().isVisible(),true);await page.locator('.group-panel').first().locator('.mobile-panel-close').click();await togglePanel('.task-panel-toggle');assert.equal(await page.locator('.task-panel').isVisible(),true);await page.locator('.task-panel .mobile-panel-close').click();}else{await togglePanel('.member-panel-toggle');assert.equal(await page.locator('.group-panel').first().isVisible(),false);assert.ok((await page.locator('.group-center').boundingBox()).width>centerWidth);await togglePanel('.task-panel-toggle');assert.equal(await page.locator('.task-panel').isVisible(),false);const center=await page.locator('.group-center').boundingBox(),message=await page.locator('.group-message.member').boundingBox(),composer=await page.locator('#requirement-form').boundingBox();assert.ok(message.width/center.width>.85);assert.ok(composer.width/center.width>.9);await togglePanel('.member-panel-toggle');await togglePanel('.task-panel-toggle');assert.equal(await page.locator('.group-panel').first().isVisible(),true);assert.equal(await page.locator('.task-panel').isVisible(),true);}
    if(width<=800)await togglePanel('.member-panel-toggle');await page.locator('.member-edit').click();await page.locator('#member-dialog').waitFor();assert.equal(await page.locator('#member-name').inputValue(),'开发成员');await page.locator('#member-cancel').click();
    ...
    await page.locator('#group-prompt').fill('较长的历史说明。'.repeat(180));await page.locator('#group-prompt').press('Enter');await page.locator('[data-message-id=sent2]').waitFor();
    await page.locator('#group-timeline').evaluate(el=>el.scrollTop=0);await page.waitForTimeout(100);
    await page.locator('#group-prompt').fill('另一位成员的新消息');await page.locator('#group-prompt').press('Enter');await page.locator('[data-message-id=sent3]').waitFor();
