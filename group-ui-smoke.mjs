...
try{
  for(const width of [390,1280]){
    ...
    await page.locator('.room-active').waitFor();
    ...
    const centerWidth=(await page.locator('.group-center').boundingBox()).width;await page.locator('.member-panel-toggle').click();assert.equal(await page.locator('.group-panel').first().isVisible(),false);if(width>800)assert.ok((await page.locator('.group-center').boundingBox()).width>centerWidth);await page.locator('.task-panel-toggle').click();assert.equal(await page.locator('.task-panel').isVisible(),false);if(width>800){const center=await page.locator('.group-center').boundingBox(),message=await page.locator('.group-message.member').boundingBox(),composer=await page.locator('#requirement-form').boundingBox();assert.ok(message.width/center.width>.85);assert.ok(composer.width/center.width>.9);}await page.locator('.member-panel-toggle').click();await page.locator('.task-panel-toggle').click();assert.equal(await page.locator('.group-panel').first().isVisible(),true);assert.equal(await page.locator('.task-panel').isVisible(),true);
    await page.locator('.member-edit').click();await page.locator('#member-dialog').waitFor();assert.equal(await page.locator('#member-name').inputValue(),'开发成员');await page.locator('#member-cancel').click();
    ...
    await page.locator('.member-edit').click();assert.equal(await page.locator('.avatar-choice.selected').getAttribute('data-value'),'preset:designer');await page.locator('#member-avatar-file').setInputFiles({name:'avatar.png',mimeType:'image/png',buffer:Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=','base64')});await page.locator('#member-avatar-preview img').waitFor();assert.match(await page.locator('#member-avatar-preview img').getAttribute('src'),/^data:image\/(webp|jpeg);base64,/);await page.locator('#member-cancel').click();
    ...
    await page.locator('.group-expand').click();assert.equal(await page.locator('.group-editor-dialog').getAttribute('open'),'');assert.equal(await page.locator('.group-editor-dialog #requirement-form').isVisible(),true);await page.getByRole('button',{name:'收起',exact:true}).click();assert.equal(await page.locator('.group-editor-dialog').isVisible(),false);await page.locator('#group-prompt').fill('');
