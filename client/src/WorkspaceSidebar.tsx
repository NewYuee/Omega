...
type Thread={id:string;updatedAt?:unknown;createdAt?:unknown;name?:string;preview?:string;unread?:boolean;unreadCount?:number;running?:boolean;omegaBinding?:{type:'member'|'coordinator';groupName:string;memberName?:string;role?:string}};
...
function Threads(){const state=useAppState(),[order,setOrder]=useState(readOrder),drag=useRef<string|null>(null),items=sortThreads(state.threads as Thread[],order),selected=state.threadId,actions=state.threadActions as unknown as ThreadActions;
  ...
  const running=items.filter(thread=>thread.running).length;
  return <><label className="thread-sort">排序 <select aria-label="会话排序" value={order.mode} onChange={event=>save({mode:event.target.value as ThreadSort,ids:order.ids.length?order.ids:items.map(item=>item.id)})}><option value="updated">最近更新</option><option value="created">最近创建</option><option value="name">名称 A–Z</option><option value="manual">自定义顺序</option></select></label>{running>0&&<small className="thread-running-count" role="status">执行中 {running}</small>}{order.mode==='manual'&&<small className="thread-sort-help">拖动 ↕ 调整，或使用上下按钮</small>}{items.map((thread,index)=>{const name=thread.name||thread.preview||'新会话';return <div className={`thread-row${thread.id===selected?' selected':''}${thread.running?' is-running':''}`} data-thread-id={thread.id} key={thread.id} onDragOver={event=>{if(order.mode==='manual'&&drag.current){event.preventDefault();event.dataTransfer.dropEffect='move'}}} onDrop={event=>{event.preventDefault();if(drag.current){move(drag.current,thread.id);drag.current=null}}}>
    ...
    <button className="thread-open" title={`${name}${thread.omegaBinding?` · ${thread.omegaBinding.groupName} · ${thread.omegaBinding.type==='coordinator'?'群组协调者':'群组成员'}`:''}${thread.running?' · 正在执行':''}`} onClick={()=>actions.open(thread.id)}><span className="sidebar-label">{name}</span>{thread.omegaBinding&&<span className="thread-binding-badge">{thread.omegaBinding.type==='coordinator'?'协调者':'群组成员'}</span>}{thread.running&&<span className="thread-running" role="status" aria-label="正在执行" title="正在执行"/>}{thread.unread&&<span className="unread-dot" aria-label={`${thread.unreadCount||1} 条未读回复`}>{Math.min(thread.unreadCount||1,99)}{(thread.unreadCount||1)>99?'＋':''}</span>}</button>
...
const widthKey='omega-sidebar-width';
const maxSidebarWidth=()=>Math.max(220,Math.min(520,window.innerWidth-360));
const clampSidebarWidth=(value:number)=>Math.min(maxSidebarWidth(),Math.max(220,value));
function savedWidth(){try{const value=Number(localStorage.getItem(widthKey));return value?clampSidebarWidth(value):260}catch{return 260}}
function WorkspaceSidebar(){const state=useAppState(),group=state.mode==='groups',actions=state.shellActions,[mobile,setMobile]=useState(()=>matchMedia('(max-width:700px)').matches),[drawerOpen,setDrawerOpen]=useState(false),[collapsed,setCollapsed]=useState(savedCollapsed);
  ...
  useEffect(()=>{const apply=()=>{document.documentElement.style.setProperty('--omega-sidebar-width',`${savedWidth()}px`)};apply();window.addEventListener('resize',apply);return()=>window.removeEventListener('resize',apply)},[]);
  const resizeStart=(event:React.PointerEvent<HTMLDivElement>)=>{if(mobile||collapsed)return;event.preventDefault();event.currentTarget.setPointerCapture(event.pointerId);document.body.classList.add('sidebar-resizing')};
  const resizeMove=(event:React.PointerEvent<HTMLDivElement>)=>{if(!event.currentTarget.hasPointerCapture(event.pointerId))return;document.documentElement.style.setProperty('--omega-sidebar-width',`${clampSidebarWidth(event.clientX)}px`)};
  const resizeEnd=(event:React.PointerEvent<HTMLDivElement>)=>{if(!event.currentTarget.hasPointerCapture(event.pointerId))return;event.currentTarget.releasePointerCapture(event.pointerId);document.body.classList.remove('sidebar-resizing');const width=parseInt(document.documentElement.style.getPropertyValue('--omega-sidebar-width'),10)||260;try{localStorage.setItem(widthKey,String(clampSidebarWidth(width)))}catch{}};
  const resizeKey=(event:React.KeyboardEvent<HTMLDivElement>)=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(event.key))return;event.preventDefault();const current=parseInt(getComputedStyle(document.documentElement).getPropertyValue('--omega-sidebar-width'),10)||260;const next=event.key==='Home'?220:event.key==='End'?maxSidebarWidth():current+(event.key==='ArrowRight'?20:-20);const width=clampSidebarWidth(next);document.documentElement.style.setProperty('--omega-sidebar-width',`${width}px`);try{localStorage.setItem(widthKey,String(width))}catch{}};
  return <>
  <div className="sidebar-resize" role="separator" aria-label="调整侧栏宽度" aria-orientation="vertical" tabIndex={0} onPointerDown={resizeStart} onPointerMove={resizeMove} onPointerUp={resizeEnd} onPointerCancel={resizeEnd} onKeyDown={resizeKey}/>
