interface RoomOptions{api(route:string,data?:unknown):Promise<any>;act?:unknown;openThread(id:string):void;error(message:string):void;onReply(target:any):void;onSelect(id:string):void}
export function createGroupRoom({api,openThread,error,onReply,onSelect}:RoomOptions){
  let group:any=null,cleared=false;
  const actions={loadWindow:async(query:string)=>{if(!group)return[];const result=await api(`groups/${encodeURIComponent(group.id)}?${query}`);return result.messages||[]},select:onSelect,openThread,reply:onReply,report:error};
