...
function Questions({requirements=[],messages=[],actions,onFocus}:{requirements?:Requirement[];messages?:Message[];actions:any;onFocus():void}){const[,tick]=useState(0);useEffect(()=>{const id=setInterval(()=>tick(value=>value+1),1000);return()=>clearInterval(id)},[]);return <>{requirements.map((requirement,index)=>{const done=['completed','accepted','awaiting_acceptance','cancelled'].includes(requirement.status),responders=[...new Set(messages.filter(message=>message.requirementId===requirement.id&&['member','coordinator'].includes(message.kind)).map(message=>message.author))];return <article className="task-card question-card" data-status={requirement.status} key={requirement.id}>
  <button type="button" className="question-link" onClick={()=>{actions.focus(requirement.id);onFocus()}}><span className="question-number">{requirements.length-index}</span>{requirement.content.slice(0,120)}</button>
  <div className="question-meta"><span className="question-state">{states[requirement.status]||'未回复'}</span><span>·</span><time className="question-duration">{elapsed(requirement.createdAt,done?requirement.updatedAt:undefined)}</time></div>
