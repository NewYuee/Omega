...
export const native = window.__OMEGA_NATIVE__ || null;
export const apiFetch = (route:string,options:RequestInit={}) => native ? native.fetch(route,options) : globalThis.fetch(route,options);
export const initialKey = () => native ? native.profile?.key || '' : sessionStorage.getItem('omega-key') || '';
export async function rememberKey(key:string) {
...
export const rememberThread = (id:string|null) => id ? localStorage.setItem(threadKey(),id) : localStorage.removeItem(threadKey());
