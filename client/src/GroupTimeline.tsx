...
type Message={id:string;kind:string;author:string;content:string;createdAt:string;requirementId?:string;reference?:Record<string,string>};
type Member={id:string;name:string;avatar?:string};
type Task={id:string;memberId:string;status:string};
type Requirement={id:string;content:string;status:string;tasks?:Task[]};
interface Model{group:{id?:string;members:Member[]};messages:Message[];requirements:Requirement[];active:Requirement[]}
interface Actions{loadWindow(query:string):Promise<Message[]>;select(id:string):void;openThread(id:string):void;reply(value:{taskId:string;name:string}):void;report(message:string):void}
...
const Timeline=forwardRef<RoomHandle,{model:Model;actions:Actions;area:HTMLElement}>(({model,actions,area},ref)=>{
  const[windowState,setWindowState]=useState<WindowState|null>(null),[followLatest,setFollowLatest]=useState(true),[focusRevision,setFocusRevision]=useState(0),focusId=useRef<string|null>(null),focusDone=useRef<((value:boolean)=>void)|null>(null),scrollTarget=useRef<'top'|'bottom'|null>('bottom');
  ...
  return <>
    ...
    {model.active.map(requirement=><div className="room-active" data-requirement-id={requirement.id} key={`active:${requirement.id}`}><span>{requirement.content.slice(0,42)} — 正在处理</span></div>)}
