...
export function FeishuPanel({read}:{read:(route:string,data?:unknown)=>Promise<unknown>}){
  ...
  const revision=useRef(''),mounted=useRef(true);
  ...
  const refresh=async(reset=false)=>{try{const s=await read('feishu') as Status;if(mounted.current)accept(s,reset);}catch(e){if(mounted.current)setError(e instanceof Error?e.message:'无法读取飞书连接状态');}};
    ...
    {status&&<>
      <form className="omega-center-form" onSubmit={e=>{e.preventDefault();const changed=appId!==status.appId&&!!status.settings.bindings.length;if(changed&&!confirm('更换飞书应用会清空原应用的会话绑定，继续？'))return;void run('connect',{appId,appSecret:secret,confirmReset:changed});}}>
        <fieldset disabled={busy} className="omega-feishu-fields"><legend>1 · 连接应用</legend>
          ...
          <div className="omega-center-actions"><button type="submit">{busy?'处理中…':'验证并保存连接'}</button>{status.enabled&&<><button type="button" onClick={()=>void run('reconnect')}>重新连接</button><button type="button" onClick={()=>{if(confirm('停用飞书消息入口？Omega Web 不受影响。'))void run('disable');}}>停用</button></>}</div>
        </fieldset>
      </form>
