...
export function FeishuPanel({read}:{read:(route:string,data?:unknown)=>Promise<unknown>}){
  ...
  return <section className="omega-integrations omega-feishu" aria-label="飞书连接器">
    ...
    <details><summary>飞书后台配置指引</summary><p>创建企业自建应用并启用机器人，开通以下权限：</p><ul>{['im:message:send_as_bot','im:message.group_at_msg:readonly','im:message.p2p_msg:readonly','im:chat:read','im:chat.members:read'].map(scope=><li key={scope}><code>{scope}</code> <button onClick={()=>void navigator.clipboard.writeText(scope).catch(()=>setError('复制失败'))}>复制</button></li>)}</ul><p>事件与回调均选择长连接。事件添加 <code>im.message.receive_v1</code>；回调添加 <code>card.action.trigger</code>。发布版本并把机器人加入目标群。若后台提示没有长连接，请先在此验证连接再返回保存。</p><p>图文图片及引用读取需开通消息读取权限（如 <code>im:message:readonly</code>），以飞书后台「获取指定消息的内容」及资源接口要求为准；权限变更后请发布应用。最多沿引用链读取 20 层，不拉取全群历史；直接图文与引用共用最多 20 个附件、合计 32 MB。</p><p>支持纯文本、图片＋文字混排提问，也可引用图片、文本/PDF/Office 文件及可读取的卡片。群内须 @机器人；执行审批仍在 Omega 处理。同一应用只运行一个连接器实例。</p></details>
  </section>;
}
