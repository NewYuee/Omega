...
function Composer({model,actions}:{model:Model;actions:Actions}){
  ...
  return <>
    ...
    <div className="compose-footer">
      <button type="button" className="icon-button" aria-label="添加附件" title="添加图片或文件 · 图片最多 4 张，每张 8 MB · 文本/代码 500 KB，PDF/Office 10 MB，与长粘贴合计最多 4 个 · 保留 7 天" disabled={locked||!model.enabled} onMouseDown={event=>{event.preventDefault();prompt.current?.saveSelection()}} onClick={()=>filePicker.current?.click()}><Icon kind="attachment"/></button>
