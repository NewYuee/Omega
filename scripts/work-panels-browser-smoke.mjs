...
try{for(const width of [390,1280]){
  ...
  await page.getByRole('button',{name:'变更',exact:true}).click();const repo=page.getByRole('dialog',{name:'仓库变更'});await repo.getByLabel('变更所属会话').selectOption('t');await repo.getByLabel('项目仓库').selectOption('omega');await repo.getByRole('button',{name:'修改 a.ts',exact:true}).click();await repo.locator('.is-added .repository-line-code').filter({hasText:'new'}).waitFor();assert.equal(await repo.locator('.is-added .repository-line-code').innerText(),'new\n');assert.equal(await repo.locator('.is-removed .repository-line-code').innerText(),'old\n');assert.doesNotMatch(await repo.locator('pre').innerText(),/diff --git|\+new|-old/);
