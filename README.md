...
- 会话列表首批 60 条；单聊历史优先通过 App Server 的 `thread/turns/list` 每页读取 60 轮摘要，并通过 `thread/items/list` 只读取当前轮最近 200 个条目。部分 thread store 不支持实验性的 item pagination 时，Omega 自动切换为 `thread/turns/list` 每页 10 轮的完整数据兼容窗口，不使用 `includeTurns` 全量加载。左右箭头可跨页，回到最新会释放旧窗口；浏览器始终只渲染一轮问答，长正文分段展示。
