...
test('member attribution shows actual source and never promotes silence to agreement',()=>{
  ...
  assert.match(text,/开发：“建议素材按账号归属”/);assert.doesNotMatch(text,/助手：“/);assert.match(text,/尚未派发或执行/);assert.match(text,/不是成员投票结果/);assert.ok(text.indexOf('当前建议')<text.indexOf('成员观点与原文依据'));
...
test('open design choices override a premature model stop, unlike pending measurements',()=>{
  const value=base();value.issues=[{kind:'discussion',question:'CE 签发还是网关续签？',reason:'现有接口取舍仍可比较',nextStep:'请两侧负责人比较兼容性',responderTaskIds:['a']}];value.continueDiscussion=false;
  ...
  for(const ids of [[],['missing'],['a','a']]){value.issues[0].responderTaskIds=ids;assert.throws(()=>parse(value),/聚焦讨论成员/);}
  value.issues[0].responderTaskIds=['a'];
...
test('pass exhaustion and budget stop retain unresolved choices without claiming consensus',()=>{
  const value=base();value.issues=[{kind:'discussion',question:'签发位置',reason:'两个方案仍需比较',nextStep:'补充接口约束',responderTaskIds:['a']}];const report=parse(value);
