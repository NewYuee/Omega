...
const clipped=(value,max=12000)=>String(value||'').slice(-max);
const wait=ms=>new Promise(resolve=>setTimeout(resolve,ms));
const directAssignments=(content,members)=>{
...
export function memberTaskPrompt(group,req,member,task){
  const mode=task.access_mode==='read'?'read':'write';
  const dependencies=new Set(JSON.parse(task.dependencies_json||'[]'));
  const handoffs=req.tasks.filter(item=>dependencies.has(item.id)&&item.status==='completed'&&item.memberId!==member.id);
  ...
  if(handoffs.length)lines.push('其他成员的必要交接：\n'+handoffs.map(item=>`${item.title}\n${clipped(item.result,8000)}`).join('\n\n'));
  return lines.join('\n\n');
}

function parseTagged(text,tag){const candidates=[];const tagged=text.match(new RegExp(`<${tag}>\\s*([\\s\\S]*?)\\s*</${tag}>`,'i'));if(tagged)candidates.push(tagged[1]);for(const match of text.matchAll(/```(?:json)?\s*([\s\S]*?)```/gi))candidates.push(match[1]);const first=text.indexOf('{'),last=text.lastIndexOf('}');if(first>=0&&last>first)candidates.push(text.slice(first,last+1));for(const candidate of candidates)try{return JSON.parse(candidate);}catch{}throw new Error('没有返回可识别的 JSON');}
export function parseCoordinatorPlan(text){const value=parseTagged(text,'omega-plan');if(typeof value.summary!=='string'||!Array.isArray(value.tasks)||!value.tasks.length)throw new Error('协调者没有返回可识别的任务计划');return value;}
export function parseTaskReview(text){const value=parseTagged(text,'omega-review');if(!['pass','rework','blocked'].includes(value.decision)||typeof value.summary!=='string')throw new Error('协调者没有返回可识别的验证结论');return value;}

export class GroupOrchestrator{
  constructor({store,startTurn,waitTurn,readTurnText,isThreadActive,isWorkspaceBusy=()=>false,notify=()=>{}}){Object.assign(this,{store,startTurn,waitTurn,readTurnText,isThreadActive,isWorkspaceBusy,notify});this.runningTasks=new Set();this.coordinatorBusy=new Set();this.pumping=new Set();this.pendingPumps=new Set();this.suspendedGroups=new Set();}
  changed(groupId){this.notify({method:'omega/group-updated',params:{groupId}});}
  submit(groupId,input){const created=this.store.createRequirement(groupId,input),group=this.store.getGroup(groupId,created.requirement.id),direct=directAssignments(created.requirement.content,group.members);if(direct.length){this.store.setDirectPlan(created.requirement.id,`直接发送给 ${direct.map(task=>group.members.find(member=>member.id===task.memberId)?.name).join('、')}`,direct);this.store.confirmPlan(groupId,created.requirement.id,true);}this.changed(groupId);this.schedule(groupId);return this.store.getGroup(groupId,created.requirement.id);}
  confirm(groupId,requirementId){const group=this.store.confirmPlan(groupId,requirementId);this.changed(groupId);this.schedule(groupId);return group;}
  retry(groupId,requirementId){const state=this.store.retry(groupId,requirementId);this.changed(groupId);this.schedule(groupId);return state.group;}
  requestChanges(groupId,requirementId,input){const group=this.store.requestChanges(groupId,requirementId,input);this.changed(groupId);this.schedule(groupId);return group;}
  resume(){for(const group of this.store.listGroups())if(group.openRequirementCount)this.schedule(group.id);}
  schedule(groupId){if(this.suspendedGroups.has(groupId))return;this.pendingPumps.add(groupId);queueMicrotask(()=>this.pump(groupId).catch(error=>console.error('[omega scheduler]',error)));}
  suspend(groupId){this.suspendedGroups.add(groupId);this.pendingPumps.delete(groupId);}
  unsuspend(groupId){this.suspendedGroups.delete(groupId);this.schedule(groupId);}
  forget(groupId){this.suspendedGroups.delete(groupId);this.pendingPumps.delete(groupId);this.coordinatorBusy.delete(groupId);this.pumping.delete(groupId);}
  async pump(groupId){if(this.suspendedGroups.has(groupId)||this.pumping.has(groupId))return;this.pumping.add(groupId);try{do{this.pendingPumps.delete(groupId);if(this.suspendedGroups.has(groupId))return;const group=this.store.getGroup(groupId);if(!this.coordinatorBusy.has(groupId)&&!this.isThreadActive(group.coordinatorThreadId)){const review=this.store.nextReview(groupId),plan=this.store.nextPlan(groupId),finalizable=this.store.nextFinalizable(groupId);const operation=review?()=>this.reviewTask(groupId,review.requirement_id,review.id):plan?()=>this.draftPlan(groupId,plan.id):finalizable?()=>this.finalize(groupId,finalizable.id):null;if(operation){this.coordinatorBusy.add(groupId);operation().catch(error=>this.handleCoordinatorError(groupId,review?.requirement_id||plan?.id||finalizable?.id,error)).finally(()=>{this.coordinatorBusy.delete(groupId);this.schedule(groupId);});}}
  ...
  handleCoordinatorError(groupId,requirementId,error){this.store.failPhase(requirementId,error.message);this.changed(groupId);}
  async startAndRead(threadId,prompt,dispatchId,timeoutMs){const started=await this.startTurn(threadId,prompt,dispatchId),turnId=started.turn?.id;if(!turnId)throw new Error('没有返回执行轮次');const completion=await this.waitTurn(threadId,turnId,timeoutMs);if(completion.status!=='completed')throw new Error(`执行状态为 ${completion.status||'未知'}`);const text=await this.readTurnText(threadId,turnId);if(!text.trim())throw new Error('没有返回可交接的结果');return{turnId,text};}
  async draftPlan(groupId,requirementId){
    const req=this.store.getRequirement(requirementId);if(!req||req.status!=='plan_drafting')return;
    if(req.replyTaskId){
      const previous=this.store.getTask(req.replyTaskId),member=this.store.getGroup(groupId).members.find(m=>m.id===previous?.memberId);
    ...
    const group=this.store.getGroup(groupId,requirementId),roster=group.members.map(m=>({id:m.id,name:m.name,role:m.role,responsibilities:m.responsibilities,project:m.projectName}));
    ...
    const plan=parseCoordinatorPlan(text);const tasks=plan.tasks.map(t=>({...t,objective:plan.tasks.length===1?req.content:t.objective,acceptance:''}));
  ...
  async finalize(groupId,requirementId){
    ...
    this.store.completeDelivery(requirementId,req.tasks.map(t=>t.result).join('\n\n'),null,false);this.changed(groupId);return;
  }
  async run(groupId,requirementId){this.schedule(groupId);const timeout=Date.now()+this.store.getGroup(groupId,requirementId).limits.taskTimeoutMinutes*60000;while(Date.now()<timeout){const req=this.store.getRequirement(requirementId);if(!req||['awaiting_confirmation','completed','accepted','cancelled','paused'].includes(req.status))return;await wait(25);}throw new Error('等待协作流程超时');}
}
